...
- Persistent notices self-dismiss when their proposals get resolved elsewhere (`registerActionNotice`/`refreshActionNotices`): a later rename that supersedes the pending batch, an inverse rename auto-cancel, or applying/skipping from the review modal all hide the stale notice. Its "Apply all" button also only applies still-pending proposals, never superseded ones.
